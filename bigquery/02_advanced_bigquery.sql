...
SELECT
    ...
    ROUND(SUM(SUM(total_order_value)) OVER (ORDER BY order_date), 2) AS cumulative_revenue
FROM `olist-logistics-warehouse.olist_gold.fact_logistics`
GROUP BY order_date
ORDER BY order_date;
...
SELECT
    dp.category,
    ROUND(AVG(fl.product_price), 2) AS avg_product_price,
    ROUND(AVG(fl.freight_value), 2) AS avg_freight_cost,
    ROUND(
        AVG(fl.freight_value) * 100.0
        / NULLIF(AVG(fl.product_price), 0), 2
    ) AS freight_as_pct_of_price
FROM `olist-logistics-warehouse.olist_gold.fact_logistics` fl
JOIN `olist-logistics-warehouse.olist_gold.dim_products` dp
    ON fl.product_id = dp.product_id
...
WITH customer_orders AS (
    ...
    FROM `olist-logistics-warehouse.olist_gold.fact_logistics` fl
    JOIN `olist-logistics-warehouse.olist_gold.dim_customers` dc
...
SELECT
    dp.category,
    ROUND(SUM(fl.total_order_value), 2) AS category_revenue,
    ROUND(
        SUM(fl.total_order_value) * 100.0
        / SUM(SUM(fl.total_order_value)) OVER(), 2
